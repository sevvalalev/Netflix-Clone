...
//  Netflix Clone
//
//  Created by Sevval Alev on 24.11.2022.
//
...
extension SearchViewController: UISearchResultsUpdating, SearchResultsViewControllerDelegate {
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        guard let query = searchBar.text,
            !query.trimmingCharacters(in: .whitespaces).isEmpty, //if its not empty !
              query.trimmingCharacters(in: .whitespaces).count >= 3,//we are trying the minimize the search
        ...
        APICaller.shared.search(with: query) { result in
            DispatchQueue.main.async {
                ...
                case .success(let titles):
                    ...
                    resultController.searchResultsCollecitonView.reloadData()
                case .failure(let error):
                    print(error.localizedDescription)
                }
            }
